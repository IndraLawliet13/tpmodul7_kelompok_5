...
public class CourseData103022300023
{
    public List<Course103022300023> courses { get; set; }
}

public class KuliahMahasiswa103022300023
{
    public void ReadJSON()
    {
        string jsonFileName = "tp7_2_103022300023.json";
        try
        {
            string jsonString = File.ReadAllText(jsonFileName);
            CourseData103022300023 dataKuliah = JsonSerializer.Deserialize<CourseData103022300023>(jsonString);
            if (dataKuliah != null && dataKuliah.courses != null)
            {
                Console.WriteLine("Daftar mata kuliah yang diambil:");
                for (int i = 0; i < dataKuliah.courses.Count; i++)
                {
                    Course103022300023 matkul = dataKuliah.courses[i];
