...
public class Course103022300002
{
    public string code { get; set; }
    public string name { get; set; }
}

public class CourseData103022300002
{
    public List<Course103022300002> courses { get; set; }
}

public class KuliahMahasiswa103022300002
{
    public void ReadJSON()
    {
        string jsonFileName = "tp7_2_103022300002.json";
        try
        {
            string jsonString = File.ReadAllText(jsonFileName);
            CourseData103022300002 dataKuliah = JsonSerializer.Deserialize<CourseData103022300002>(jsonString);
            if (dataKuliah != null && dataKuliah.courses != null)
            {
                Console.WriteLine("Daftar mata kuliah yang diambil:");
                for (int i = 0; i < dataKuliah.courses.Count; i++)
                {
                    Course103022300002 matkul = dataKuliah.courses[i];
